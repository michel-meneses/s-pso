...
int contaAtributos (FILE* input){
	...
	while(!feof(input)){
		...
		const tam_str_aux = 1000;																								// define o tamanho máximo de uma linha a ser lida;	
...
char* saltaStringArq ( char* string, FILE* arq ){
	...
	const tam_str_aux = 200;																								// define o tamanho máximo de uma linha a ser lida;
	char str_aux [tam_str_aux];																								// string auxiliar que receberá a linha do arquivo .arff a ser lida;
...
void buscaLocal( parametros param, atributo* atrib, int quant_atrib, exemplo* exemplos, int quant_exemp ){
	...
	regra atual, vizinho;
	int i,j,sair;																											// caso durante uma interação nenhum vizinho for melhor que a regra atual o algoritmo é encerrado;
...
regra mapeiaLinhaWeka(atributo* atributos, int quant_atrib, char* linha){
	...
	int indice_atributo = 0;																									// representa o índice do atributo atual da linha sendo analisado;
	int indice_letra = 0;																										// representa a letra atual da linha sendo analisada;	
	int tamanho_aux = 200;
	char aux[tamanho_aux];																										// auxiliar que receberá as palavras analisadas da linha;
	int indice_aux = 0;																											// índice da letra atual analisada de aux;
	char* str_aux;																												// string dinâmica que receberá o valor de aux;
	int atributo = -1;																											// índice do atributo atual sendo analisado;																												
	int j;
	int flag = 0;																												// indica se todos os atributos antes da classe já foram verificados;
	
	//Trata dos atributos anteriores à classe
...
regra* getRegrasWeka(atributo* atributos, int quant_atrib, int* quant_regras){
	...
	if (weka == NULL){																											// caso o  arquivo não exista
		
		printf("Arquivo Weka não encontrado!\n");																				// emite mensagem de erro
		exit(1);																												// e finaliza;
	}
	...
	*quant_regras = 0;
	...
	int indice = 0;
	while(!feof(weka)){
		char str_aux [tam_str_aux];																								// string auxiliar que receberá a linha do arquivo Weka a ser lida;
...
void calculaObjetivosRegrasWeka(atributo* atrib, int quant_atrib, exemplo* exemplos, int quant_exemp, parametros param){
	...
	FILE* arq_solucao_weka = fopen("soluções_weka.txt", "w");
